      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          {creationRank?.racewar_users?.length > 0 && (
            <section>
              <div className="text-xs font-mono text-white/30 mb-3">创作指数排行</div>
              <div className="space-y-2">
                {creationRank.racewar_users.slice(0, 10).map((u: any, i: number) => {
          ...
          {/* 数字人排行 */}
          {digitalRank?.records?.length > 0 && (
            <section>
              <div className="text-xs font-mono text-white/30 mb-3">数字人排行</div>
              <div className="space-y-2">
                {digitalRank.records.slice(0, 8).map((u: any, i: number) => {
                  const color = RACE_COLORS[u.user_race] ?? '#888'
                  return (
                    <div key={u.seq} className="flex items-center gap-2 py-1.5 border-b border-white/5 last:border-0">
                      <span className="text-xs font-mono text-white/20 w-4">{i + 1}</span>
